...
class SolicitudController extends Controller
{
    ...
    public function list(Request $request){
        ...
        /* $q = DB::table('solicitud as so')
            ...
            ->leftJoin('users as us2', 'us2.id', '=', 'so.idfuncionarioresponde')
            ->join('tipo_prioridad as tp', 'tp.idprioridad', '=', 'so.idprioridad')
            ->leftJoin('categorias_solicitudes as cs', 'cs.id', '=', 'so.id_categoria')
            ->leftJoin('satisfaccion as sa', 'sa.idsatisfaccion', '=', 'so.idsatisfaccion')
            ->where('so.idarea', 1)
            ->selectRaw("
                so.idsolicitud,
                s.descsede as sede,
                so.asunto,
                so.fechahora_solicitud,
                so.idestado_solicitud,
                so.horasolicitud,
                so.fechahora_visita,
                so.horavisita,
                sa.desc_satisfaccion,
                so.idsatisfaccion,
                s.descsede,
                est.descestado_solicitud,
                us.name as nombre,
                so.idfuncionario,
                tp.desc_prioridad,
                us2.name as asignado_a,
                so.idfuncionarioresponde,
